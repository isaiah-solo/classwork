...
void do_debug (bigint_stack& stack, const char)
{
   ...
   cout << "ydc: Y not implemented" << endl;
}
...
void scan_options (int argc, char** argv)
{
   ...
   for (;;)
   {
      ...
      switch (option)
      {
         case '@':
            ...
            break;
         default:
            complain() << "ydc: " << "-" << (char) optopt << ": invalid option"
                       << endl;
            break;
      }
   }
   if (optind < argc)
   {
      complain() << "ydc: operand not permitted" << endl;
   }
}

//
            ...
            switch (token.symbol)
            {
               ...
               case OPERATOR:
               {
                  ...
                  if (fn == do_functions.end())
                     throw ydc_exn (octal (token.lexinfo[0]) + " is unimplemented");
